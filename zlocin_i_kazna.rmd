...
Neka je $A = \text{contingency_table}$ i numerirajmo retke i stupce tablice A brojevima i={1,2,3} odnosno j={1,2,3,4}. <br>
...
Pearsonov rezidual jest razlika dobivene i uočene vrijednosti normirane po korijenu očekivane.
$r_ij = \frac{o_ij - e_ij}{\sqrt{e_ij}}$ kvadrati residuala doprinose vrijednosti testne statistike i time smanjuju p-vrijednost. Zato možemo reći sljedeće: <br>
Kada je $r_ij$ > 0 (dovoljno veći od 0) kažemo da postoji asocijacija između retka i stupca jer je više uočeno nego očekivano, a dok je $r_ij$ < 0 onda kažemo da ne postoji asocijacija (ili postoji negativna asocijacija) jer je manje uočeno nego očekivano.
...
Ako ćemo gledati samo zloćine zbog kojih su osuđenici zatvoreni, tada koristimo stupac "Prison_Offense" koji poprima 5 vrijednosti: Violent/Sex, Violent/Non-Sex,
Property, Drug, Other. <br>

Time možemo provesti 1*4=4 testova.
...
Pogledajmo reziduale za kokain i thc.

```{r}
corrplot(cocaine_test$residuals, is.corr=FALSE)
```

Uočavamo da postoji povezanost između "property" kriminala i čestog i vrlo čestog korištenja kokaina.

```{r}
corrplot(thc_test$residuals, is.corr=FALSE)
```

Vidimo da postoji povezanost između "property" kriminala i korištenja THC-a. Također vidimo da ne postoji povezanost između "Violent/Sex" i korištenja THC-a, isto tako za "Drug" kriminal.
...
ANOVA se provodi uz pretpostavku normalnosti distribucije reziduala i da su varijance među grupama (stupanj obrazovanja) iste. <br>
...
```
Iz box-plota bismo mogli naslutiti da bi zatvorenici sa manjim stupnjom obrazovanja mogli konzumirati više droga. Pogledajmo srednje vrijednosti: <br>
...
**Zbog toga što se većina zatvorenika ne drogira, izdvojit ćemo one koji koriste narkotike i nad njima ćemo provoditi testove.**
...
Pogledajmo srednje vrijednosti testova na droge među populacijom koja ima "drugs" > 0.
```{r}
...
recidivism_full_filtered <- subset(recidivism_full_filtered, select=c("Education_Level","boxcox"))
```
...
ggplot(recidivism_full_filtered, aes(x = boxcox, fill = Education_Level)) +
  ...
  ylab("Count")
```
Distribucije izgledaju puno bolje. Pogledajmo statističkim testom jesu li normalni. <br>

```{r}
shapiro.test(sample(recidivism_full_filtered$boxcox, size=5000, replace=TRUE))
```
...
varijance
```
Vidimo da su varijance otprilike iste. Približno jednake 1.<br>
...
summary(model)
```
p-vrijednost ANOVA-e jest manja od $2\dot 10^{-16}$.
**Stoga statistički zaključujemo uz značaj 0.05 da se među zatvorenicima koji konzumiraju narkotike (prosjek veći od 0) prosječna vrijednost pozitivnih testova na droge nije jednaka za svaki stupanj obrazovanja**
...
```{r}
shapiro.test(sample(residuals, size=5000, replace=TRUE))
```
...
qqline(residuals)
```
QQ-plotom vidimo da su podaci "većinom" normalni osim devijacija na desnom repu.

Postoji naćin da testiramo iste hipoteze neovisno o distribuciji podataka. Takve metode zovemo neparametarskim i neparametarski ekvivalent jednofaktorskoj ANOVA-i je Kruskal-Wallisov test.
```{r}
test <- kruskal.test(boxcox ~ Education_Level, data = recidivism_full_filtered)
print(test)
```
Kruskal-Wallisovim testom opet potvrđujemo zaključak od maloprije.
