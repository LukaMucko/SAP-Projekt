...
Iako ne možemo uspoređivati aritmetičke sredine log-transformiranih podataka i originalnih, ali zbog izrazito male p-vrijednosti možemo reći da postoji razlika između aritmetičke sredine
...
Provedimo još jednom t-test na netransformiranim podaci da vidimo tko ima veću srednju vrijednost. Pretpostavit ćemo da mladi imaju veći.
```{r}
t.test(mladi_drogerasi$total, stari_drogerasi$total, var.equal=TRUE, alternative = "less")
```
Vidimo da uz p-vrijednost 1 prihvaćamo alternativu "less" tj. razlika između srednje vrijednosti mladih i starih je manje od 0. Mladi osuđenici koji konzumiraju narkotike konzumiraju više.
...
```{r}
osoba <- data.frame(Prior_Arrest_Episodes_PPViolationCharges ="0")
p<- predict(model, osoba,type="response" )
p
```
...
**Stoga statistički zaključujemo uz značaj 0.05 da se među zatvorenicima koji konzumiraju narkotike prosječna vrijednost pozitivnih testova na droge nije jednaka za svaki stupanj obrazovanja**
...
Postoji naćin da testiramo iste hipoteze neovisno o distribuciji podataka. Takve metode zovemo neparametarskim i neparametarski ekvivalent jednofaktorskoj anovi je Kruskal-Wallisov test.
Kruskal-Wallisov test zahtijeva da podaci dolaze iz iste distribucije, a hipoteze su mu:
