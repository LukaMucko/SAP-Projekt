...
Sada možemo provesti T-test uz nejednake varijance. Poznavanje varijanci nam određuje broj sloboda i testnu statistiku.
...
Procjenjujemo da je vjerojatnost recidivizma za osobu koja nema završeni fakultet ~61% veća od vjerojatnosti za osobu koja ima neki završeni neki fakultet 45%. <br>
...
Zbog toga što se većina osuđenika ne konzumira narkotike, izdvojit ćemo one koji koriste narkotike i nad njima ćemo provoditi testove.
...
ggplot(recidivism_full_filtered, aes(x = boxcox, fill = Education_Level)) +
  ...
  ylab("Count")
```
Distribucije izgledaju normalnije. Pogledajmo statističkim testom jesu li normalni. <br>
...
qqline(residuals)
```
QQ-plotom vidimo da su podaci "većinom" normalni osim devijacija na desnom repu, ali ćemo ipak uzeti p-vrijednost ANOVA-e kao valjan rezultat.
