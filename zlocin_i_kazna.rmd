...
Kada je $r_{ij}$ > 0 kažemo da postoji pozitivna asocijacija između retka i stupca jer je više uočeno nego očekivano, a dok je $r_{ij}$ < 0 onda kažemo postoji negativna asocijacija jer je manje uočeno nego očekivano. <br>
...
```{r}
chisq<-chisq.test(recidivism_full$Prior_Conviction_Episodes_Misd, recidivism_full$THCUsage,simulate.p.value = TRUE)
chisq
```
...
Plavo označava da više zatvorenika od očekivanog koji često konzumiraju THC osuđeno zbog misdemeanora (vandalizam, pijana, posjedovanje droge!). Tj. da postoji pozitivna povezanost između nasilja i konzumiranja THC-a. <br>
...
Time možemo provesti 4 testova.
...
Vidimo da postoji pozitivna povezanost između "property" kriminala i korištenja THC-a. Također vidimo da ne postoji pozitivna povezanost između "Violent/Sex" i korištenja THC-a, isto tako za "Drug" kriminal.
...
var_mladi <- var(mladi_drogerasi)
var_stari <- var(stari_drogerasi)
```
...
ggplot() +
  geom_histogram(data = mladi_drogerasi_trans, aes(x = total_trans), fill = "blue", alpha = 0.5) +
  geom_histogram(data = stari_drogerasi_trans, aes(x = total_trans), fill = "red", alpha = 0.5) +
...
```
Zbog značanjih devijacija od pravca ne možemo reći da su podaci normalno distribuirani. <br>
...
Sada možemo provesti T-test uz nejednake varijance. Poznavanje varijanci nam određuje broj sloboda i testntu statistiku.
Budući da smo podatke transformirali logaritmom hipoteze će sada glasiti:

$H_0$: geometrijske sredine starih i mladih su iste.

$H_1$: geometrijske sredine starih i mladih nisu iste.
...
Razlike u aritmetičkoj sredini transformiranih podataka implicira razliku u aritmetičkoj sredini izvornih podataka. Stoga možemo statistički zaključiti da aritmetičke sredine među mladim osuđenicima koji koriste narkotike nije ista kao kod starih.

Budući da je broj uzoraka izrazito velik, još jednom ćemo se uzdat u robusnost T-testa te ga provesti na netransformiranim podacima.
...
```
Ponovno odbacujemo nultu hipotezu o jednakosti srednjih vrijednosti.
...
model$coefficients
```
Vidimo da osoba koja ima što više kršenja Parolea prije osude ima veću šansu da ponovni recidivizam.
...
ANOVA se provodi uz pretpostavku normalnosti distribucije reziduala, da su varijance među grupama (stupanj obrazovanja) iste i nezavisnost grupa. <br>
...
```{r}
recidivism_full_filtered <- recidivism_full_filtered %>% subset(select=c("Education_Level","drugs")) %>% na.omit()
...
```
Statistički zaključujemo da podaci nisu normalni.

```{r}
qqnorm(residuals)
qqline(residuals)
```
QQ-plotom vidimo da su podaci "većinom" normalni osim devijacija na desnom repu, stoga je korištenje ANOVA-e opravdano
...
Pogledajmo stvarne vrijednosti srednjih vrijednosti:
```{r}
means <- aggregate(drugs ~ Education_Level, data = recidivism_full_filtered, mean)
means
```
