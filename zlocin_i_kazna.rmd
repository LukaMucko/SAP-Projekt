...
Sada možemo provesti ukupno 4*8= 32 testa. Koristimo simulate.p.value = TRUE što Monte Carlo simulacijom simulira p-vrijednost, to koristimo jer neke očekivane vrijednosti nisu veće od 5.<br>
...
Plavo označava da više zatvorenika od očekivanog koji često konzumiraju THC osuđeno zbog misdemeanora (vandalizam, vožnje pod utjecajem alkohola, posjedovanje droge! i sl.). Tj. da postoji pozitivna povezanost između nasilja i konzumiranja THC-a. <br>
...
stari_drogerasi <- stari_drogerasi %>%
  ...
  select(-DrugTests_THC_Positive,-DrugTests_Meth_Positive,-DrugTests_Cocaine_Positive,-DrugTests_Other_Positive) %>% na.omit()
...
Time dobivamo funkciju $p(x) = f(x^T \mathbf{\beta})$ koja daje vrijednosti između $\langle 0, 1 \rangle$. Budući da trebamo binarne vrijednosti "TRUE" i "FALSE" za $x_i$ koji daju $p>0.5$ označit ćemo s "TRUE", inaće "FALSE". <br>
...
Koristimo neparametarski test. Neparametarski ekvivalent jednofaktorskoj anovi je Kruskal-Wallisov test.
hipoteze su:
...
# Zaključak
Pokazali smo da postoji veza između duljine zatvorske kazne s obzirom na razinu obrazovanja osuđenici da većim stupnjem obrazovanja su osuđeni na više godina.
Mlađe osobe su podložnije ponavljanju kriminala. Postoje povezanosti između nekih droga i zločina, ali ne za svaku kombinaciju. Statistički smo zaključili da ne postoji povezanost između počinjenog zločina i konzumiranja metamfetamina.
Mlađe osobe su također sklonije narkoticima. Dali smo par primjera predikcije recidivizma pomoću logističke regresije. Procijenili smo da vjerojatnost recidivizma veća za osuđenike s manjim stupnjem obrazovanja. I pokazali smo uz pomoć anove da konzumiranje narkotika među skupinama s različitim stupnjem obrazovanja nije jednako.